  ...
  try {
    for (const appId of appIds) {
        dbOperations.getApplicationById(appId, (err, application) => {
            ...
            // Traitez les données de l'application et renvoyez la réponse appropriée
      ...
      //   return res
      //     .status(404)
      //     .json({ error: `Application with ID ${appId} not found` });
      // }
      ...
      return new Promise((resolve, reject) => {
        ...
        exec(installCommand, (error, stdout, stderr) => {
          if (error) {
            console.error(
              `Erreur lors de l'installation de ${appName} : ${error.message}`
            );
            reject(error);
          } else {
